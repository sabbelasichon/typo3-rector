...
final class FullQualifiedNamePostRector extends AbstractPostRector
{
    ...
    {
        return new RuleDefinition('Use fully qualified names', [
            new CodeSample(
                <<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    private function shouldSkip(File $file): bool
    {
        ...
        $filesAndDirectories = $this->parameterProvider->provideArrayParameter(
            Typo3Option::PATHS_FULL_QUALIFIED_NAMESPACES
        );
    ...
    private function changeNameImportingPostRectorSkipConfiguration(ParameterProvider $parameterProvider): void
    {
        ...
        $useFullQualifiedClassNamePaths = $parameterProvider->provideArrayParameter(
            Typo3Option::PATHS_FULL_QUALIFIED_NAMESPACES
        );
        ...
        $skipConfiguration = $parameterProvider->hasParameter(Option::SKIP) ? $parameterProvider->provideArrayParameter(
            Option::SKIP
        ) : [];

        $nameImportingPostRectorConfiguration = array_search(NameImportingPostRector::class, $skipConfiguration, true);

        // Do nothing because NameImportingPostRector is skipped totally
        if (false !== $nameImportingPostRectorConfiguration && ! is_array(
            $skipConfiguration[$nameImportingPostRectorConfiguration]
        )) {
            return;
        }
        ...
        $skipConfiguration[NameImportingPostRector::class] = array_merge(
            $skipConfiguration[NameImportingPostRector::class],
            $useFullQualifiedClassNamePaths
        );
