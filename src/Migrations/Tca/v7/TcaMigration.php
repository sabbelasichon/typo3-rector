...
namespace Ssch\TYPO3Rector\Migrations\Tca\v7;
...
final class TcaMigration implements \Ssch\TYPO3Rector\Migrations\Tca\TcaMigration
{
     ...
     * @return array Migrated TCA
     */
    private function migrateT3editorWizardToRenderTypeT3editorIfNotEnabledByTypeConfig(array $tca)
    {
        $newTca = $tca;
        foreach ($tca as $table => $tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
     ...
     */
    private function migrateSpecialConfigurationAndRemoveShowItemStylePointerConfig(array $tca)
    {
        $newTca = $tca;
        foreach ($tca as $table => $tableDefinition) {
     ...
     * @return array Migrated TCA
     */
    private function migrateT3editorWizardWithEnabledByTypeConfigToColumnsOverrides(array $tca)
    {
        $newTca = $tca;
        foreach ($tca as $table => $tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
     ...
     * @return array Migrated TCA
     */
    private function migrateShowItemAdditionalPaletteToOwnPalette(array $tca)
    {
        $newTca = $tca;
        foreach ($tca as $table => $tableDefinition) {
            if (!isset($tableDefinition['types']) || !is_array($tableDefinition['types'])) {
     ...
     * @return array Migrated TCA
     */
    private function migrateIconsForFormFieldWizardsToNewLocation(array $tca)
    {
        $newTca = $tca;
     ...
     * @return array Migrated TCA
     */
    private function migrateExtAndSysextPathToEXTPath(array $tca)
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
     ...
     * @return array Migrated TCA
     */
    private function migrateIconsInOptionTags(array $tca)
    {
        $newTca = $tca;
     ...
     * @return array Migrated TCA
     */
    private function migrateIconfileRelativePathOrFilenameOnlyToExtReference(array $tca)
    {
        foreach ($tca as $table => &$tableDefinition) {
     ...
     * @return array Migrated TCA
     */
    private function migrateElementBrowserWizardToLinkHandler(array $tca)
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
