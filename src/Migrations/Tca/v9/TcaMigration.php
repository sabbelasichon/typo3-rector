...
namespace Ssch\TYPO3Rector\Migrations\Tca\v9;

use TYPO3\CMS\Core\Utility\GeneralUtility;

final class TcaMigration implements \Ssch\TYPO3Rector\Migrations\Tca\TcaMigration
{
    /**
    ...
    public function migrateSelectFieldRenderType(array $tca): array
    {
        ...
        foreach ($newTca as $table => &$tableDefinition) {
            ...
            foreach ($tableDefinition['columns'] as $columnName => &$columnDefinition) {
                ...
                if (empty($columnDefinition['config']['type']) || 'select' !== $columnDefinition['config']['type']) {
                    continue;
                }
    ...
    public function migrateSelectFieldIconTable(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                if (empty($fieldConfig['config']['renderType']) || 'selectSingle' !== $fieldConfig['config']['renderType']) {
                    continue;
                }
                        ...
                        if (isset($wizardConfig['enableByTypeConfig']) // and enableByTypeConfig is given
                            ...
                            foreach ($tableDefinition['types'] as $typeName => $typeArray) {
                                if (empty($typeArray['columnsOverrides'][$fieldName]['defaultExtras'])
                                    || false === strpos($typeArray['columnsOverrides'][$fieldName]['defaultExtras'], $wizardName)
                                ) {
                                ...
                                foreach ($defaultExtrasArray as $fieldExtraField) {
                                    if (0 === strpos($fieldExtraField, 'wizards[')) {
     ...
     * @param array $tca Incoming TCA
     */
    private function validateTcaType(array $tca): void
    {
        foreach ($tca as $table => $tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
     ...
     * @return array
     */
    private function migrateColumnsConfig(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
     ...
     * @return array Migrated TCA
     */
    private function migrateT3editorWizardToRenderTypeT3editorIfNotEnabledByTypeConfig(array $tca): array
    {
        $newTca = $tca;
        foreach ($tca as $table => $tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
            ...
            foreach ($tableDefinition['columns'] as $fieldName => $fieldConfig) {
                if (
                    !empty($fieldConfig['config']['type']) // type is set
                    && 'text' === trim($fieldConfig['config']['type']) // to "text"
                    && isset($fieldConfig['config']['wizards'])
                    && is_array($fieldConfig['config']['wizards']) // and there are wizards
                ) {
                    foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                        if (
                            ...
                            && 'TYPO3\\CMS\\T3editor\\FormWizard->main' === trim($wizardConfig['userFunc']) // and set to FormWizard
                            && (
     ...
     */
    private function migrateSpecialConfigurationAndRemoveShowItemStylePointerConfig(array $tca): array
    {
        $newTca = $tca;
        foreach ($tca as $table => $tableDefinition) {
            if (!isset($tableDefinition['types']) || !is_array($tableDefinition['types'])) {
            ...
            foreach ($tableDefinition['types'] as $typeName => $typeArray) {
                if (!isset($typeArray['showitem']) || !is_string($typeArray['showitem']) || false === strpos($typeArray['showitem'], ';')) {
     ...
     * @return array Migrated TCA
     */
    private function migrateT3editorWizardWithEnabledByTypeConfigToColumnsOverrides(array $tca): array
    {
        $newTca = $tca;
        foreach ($tca as $table => $tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
     ...
     * @return array Migrated TCA
     */
    private function migrateShowItemAdditionalPaletteToOwnPalette(array $tca): array
    {
        $newTca = $tca;
        foreach ($tca as $table => $tableDefinition) {
            if (!isset($tableDefinition['types']) || !is_array($tableDefinition['types'])) {
            ...
            foreach ($tableDefinition['types'] as $typeName => $typeArray) {
                if (
                    ...
                    || false === strpos($typeArray['showitem'], ';') // no field parameters
                ) {
                    continue;
                }
                $itemList = GeneralUtility::trimExplode(',', $typeArray['showitem'], true);
                ...
                foreach ($itemList as $fieldString) {
                    ...
                    if ('--palette--' !== $fieldArray['fieldName'] && null !== $fieldArray['paletteName']) {
     ...
     * @return array Migrated TCA
     */
    private function migrateIconsForFormFieldWizardToNewLocation(array $tca): array
    {
        $newTca = $tca;
                ...
                if (
                    ...
                    foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                        ...
                        foreach ($wizardConfig as $option => $value) {
                            if ('icon' === $option && in_array($value, $oldFileNames, true)) {
     ...
     * @return array Migrated TCA
     */
    private function migrateExtAndSysextPathToEXTPath(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                if (
                    !empty($fieldConfig['config']['type']) // type is set
                    && 'select' === trim($fieldConfig['config']['type']) // to "select"
                    ...
                    foreach ($fieldConfig['config']['items'] as &$itemConfig) {
                        ...
                        if (!empty($itemConfig[2])) {
                            ...
                            if (
                                0 === strpos($itemConfig[2], 'ext/')
                                || 0 === strpos($itemConfig[2], 'sysext/')
                            ) {
                            ...
                            } elseif (0 === strpos($itemConfig[2], 'i/')) {
     ...
     * @return array Migrated TCA
     */
    private function migrateIconsInOptionTags(array $tca): array
    {
        $newTca = $tca;
     ...
     * @return array Migrated TCA
     */
    private function migrateIconfileRelativePathOrFilenameOnlyToExtReference(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            ...
            if (0 === strpos($tableDefinition['ctrl']['iconfile'], '../typo3conf/ext/')) {
            ...
            } elseif (false === strpos($tableDefinition['ctrl']['iconfile'], '/')) {
     ...
     * @return array Migrated TCA
     */
    private function migrateElementBrowserWizardToLinkHandler(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                if (
                    isset($fieldConfig['config']['wizards']['link']['module']['name']) && 'wizard_element_browser' === $fieldConfig['config']['wizards']['link']['module']['name']
                    && isset($fieldConfig['config']['wizards']['link']['module']['urlParameters']['mode']) && 'wizard' === $fieldConfig['config']['wizards']['link']['module']['urlParameters']['mode']
                ) {
     ...
     * @return array Migrated TCA
     */
    private function migrateDefaultExtrasRteTransFormOptions(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                if (isset($fieldConfig['defaultExtras'])) {
                    ...
                    foreach ($defaultExtrasArray as $defaultExtrasField) {
                        if (0 === strpos($defaultExtrasField, 'richtext')) {
     ...
     * @return array Migrated TCA
     */
    private function migrateSelectTreeOptions(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                if (isset($fieldConfig['config']['renderType']) && 'selectTree' === $fieldConfig['config']['renderType']) {
     ...
     * @return array Migrated TCA
     */
    private function migrateTSconfigSoftReferences(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
     ...
     * @return array Migrated TCA
     */
    private function migrateShowIfRteOption(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                if (isset($fieldConfig['config']) && 'check' === $fieldConfig['config']['type']) {
     ...
     * @return array Migrated TCA
     */
    private function migrateWorkspacesOptions(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
     ...
     * @return array Migrated TCA
     */
    private function migrateTranslationTable(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
     ...
     * @return array Migrated TCA
     */
    private function migrateL10nModeDefinitions(array $tca)
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
     ...
     * @return array Migrated TCA
     */
    private function migratePageLocalizationDefinitions(array $tca)
    {
        if (
        ...
        foreach ($tca['pages_language_overlay']['columns'] as $fieldName => &$fieldConfig) {
            ...
            if (null !== $l10nMode) {
                if (!empty($oppositeFieldConfig) && 'exclude' !== $oppositeL10nMode) {
     ...
     * @return array
     */
    private function migrateInlineLocalizationMode(array $tca)
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                if ('inline' !== ($fieldConfig['config']['type'] ?? null)) {
                    continue;
                }

                $inlineLocalizationMode = ($fieldConfig['config']['behaviour']['localizationMode'] ?? null);
                if (null === $inlineLocalizationMode) {
                    continue;
                }

                $allowLanguageSynchronization = ($fieldConfig['config']['behaviour']['allowLanguageSynchronization'] ?? null);
                if ('keep' === $inlineLocalizationMode && $allowLanguageSynchronization) {
     ...
     * @return array Migrated TCA
     */
    private function migrateRequestUpdate(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
     ...
     * @return array Migrated TCA
     */
    private function migrateInputDateTimeToRenderType(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                if (isset($fieldConfig['config']['type']) && 'input' !== $fieldConfig['config']['type']) {
                    continue;
                }
     ...
     * @return array Migrated TCA
     */
    private function migrateColorPickerWizardToRenderType(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config'])) {
                        ...
                        if ('input' === $fieldConfig['config']['type']) {
                            if (isset($fieldConfig['config']['wizards']) && is_array($fieldConfig['config']['wizards'])) {
                                foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizard) {
                                    if (isset($wizard['type']) && ('colorbox' === $wizard['type'])) {
     ...
     * @return array Migrated TCA
     */
    private function migrateSelectWizardToValuePicker(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && ('input' === $fieldConfig['config']['type'] || 'text' === $fieldConfig['config']['type'])) {
                        if (isset($fieldConfig['config']['wizards']) && is_array($fieldConfig['config']['wizards'])) {
                            foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                                if (isset($wizardConfig['type'])
                                    && 'select' === $wizardConfig['type']
     ...
     * @return array Migrated TCA
     */
    private function migrateSliderWizardToSliderConfiguration(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && 'input' === $fieldConfig['config']['type']) {
                        if (isset($fieldConfig['config']['wizards'])
                            ...
                            foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                                if (isset($wizardConfig['type']) && 'slider' === $wizardConfig['type']) {
                            ...
                            foreach ($tableDefinition['types'] as $typeName => &$typeArray) {
                                if (isset($typeArray['columnsOverrides']) && is_array($typeArray['columnsOverrides'])) {
                                    if (isset($typeArray['columnsOverrides'][$fieldName]['config']['wizards'])
                                        ...
                                        foreach ($typeArray['columnsOverrides'][$fieldName]['config']['wizards'] as $wizardName => $wizard) {
                                            if (isset($wizard['type']) && ('slider' === $wizard['type'])) {
     ...
     * @return array Modified TCA
     */
    private function migrateLinkWizardToRenderTypeAndFieldControl(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && 'input' === $fieldConfig['config']['type']
                        ...
                        if (isset($fieldConfig['config']['wizards'])
                            ...
                            foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                                if (isset($wizardConfig['type'])
                                    && 'popup' === $wizardConfig['type']
                                    && isset($wizardConfig['module']['name'])
                                    && 'wizard_link' === $wizardConfig['module']['name']
                                ) {
                                    ...
                                    if (isset($typeArray['columnsOverrides'][$fieldName]['config']['wizards'])
                                        ...
                                        foreach ($typeArray['columnsOverrides'][$fieldName]['config']['wizards'] as $wizardName => $wizard) {
                                            if (isset($wizard['type'])
                                                ...
                                                && 'wizard_link' === $wizard['module']['name']
                                            ) {
     ...
     * @return array
     */
    private function migrateEditWizardToFieldControl(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && ('group' === $fieldConfig['config']['type']
                                                                  || 'select' === $fieldConfig['config']['type'])
                    ) {
                        if (isset($fieldConfig['config']['wizards'])
                            && is_array($fieldConfig['config']['wizards'])
                        ) {
                            foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                                if (isset($wizardConfig['type'])
                                    && 'popup' === $wizardConfig['type']
                                    && isset($wizardConfig['module']['name'])
                                    && 'wizard_edit' === $wizardConfig['module']['name']
                                    ...
                                    if (isset($typeArray['columnsOverrides'][$fieldName]['config']['wizards'])
                                        ...
                                        foreach ($typeArray['columnsOverrides'][$fieldName]['config']['wizards'] as $wizardName => $wizard) {
                                            if (isset($wizard['type'])
                                                && 'popup' === $wizard['type']
                                                && isset($wizard['module']['name'])
                                                && 'wizard_edit' === $wizard['module']['name']
                                            ) {
     ...
     * @return array
     */
    private function migrateAddWizardToFieldControl(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && ('group' === $fieldConfig['config']['type']
                        ...
                        if (isset($fieldConfig['config']['wizards'])
                            ...
                            foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                                if (isset($wizardConfig['type'])
                                    && 'script' === $wizardConfig['type']
                                    && isset($wizardConfig['module']['name'])
                                    && 'wizard_add' === $wizardConfig['module']['name']
     ...
     * @return array
     */
    private function migrateListWizardToFieldControl(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                        ...
                        if (isset($fieldConfig['config']['wizards'])
                            ...
                            foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                                if (isset($wizardConfig['type'])
                                    && 'script' === $wizardConfig['type']
                                    && isset($wizardConfig['module']['name'])
                                    && 'wizard_list' === $wizardConfig['module']['name']
                                    ...
                                    if (isset($typeArray['columnsOverrides'][$fieldName]['config']['wizards'])
                                        ...
                                        foreach ($typeArray['columnsOverrides'][$fieldName]['config']['wizards'] as $wizardName => $wizard) {
                                            if (isset($wizard['type'])
                                                && 'script' === $wizard['type']
                                                && isset($wizard['module']['name'])
                                                && 'wizard_list' === $wizard['module']['name']
                                            ) {
     ...
     * @return array
     */
    private function migrateLastPiecesOfDefaultExtras(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['defaultExtras'])) {
                        ...
                        foreach ($defaultExtrasArray as $defaultExtrasSetting) {
                            if ('rte_only' === $defaultExtrasSetting) {
     ...
     * @return array
     */
    private function migrateTableWizardToRenderType(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                        ...
                        if (isset($fieldConfig['config']['wizards'])
                            ...
                            foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                                if (isset($wizardConfig['type'])
                                    && 'script' === $wizardConfig['type']
                                    && isset($wizardConfig['module']['name'])
                                    && 'wizard_table' === $wizardConfig['module']['name']
                                    ...
                                    if (isset($wizardConfig['params']['xmlOutput']) && 0 !== (int) $wizardConfig['params']['xmlOutput']) {
                                    ...
                                    if (isset($typeArray['columnsOverrides'][$fieldName]['config']['wizards'])
                                        ...
                                        foreach ($typeArray['columnsOverrides'][$fieldName]['config']['wizards'] as $wizardName => $wizard) {
                                            if (isset($wizard['type'])
                                                && 'script' === $wizard['type']
                                                && isset($wizard['module']['name'])
                                                && 'wizard_table' === $wizard['module']['name']
                                                ...
                                                if (isset($wizard['params']['xmlOutput']) && 0 !== (int) $wizard['params']['xmlOutput']) {
     ...
     * @return array
     */
    private function migrateFullScreenRichtextToFieldControl(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                            ...
                            foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                                if (isset($wizardConfig['type'])
                                    ...
                                    $this->messages[] = 'The RTE fullscreen wizard in TCA has been migrated to a \'fieldControl\' = \'fullScreenRichtext\'.'
                                          ...
                                          && (
                                              !isset($fieldConfig['config']['enableRichtext'])
                                              || isset($fieldConfig['config']['enableRichtext']) && false === (bool) $fieldConfig['config']['enableRichtext']
                                          )
                                ) {
                                        ...
                                        foreach ($tableDefinition['types'] as $typeName => &$typeArray) {
                                            if (isset($typeArray['columnsOverrides']) && is_array($typeArray['columnsOverrides'])) {
                                                if (isset($typeArray['columnsOverrides'][$fieldName]['config']['enableRichtext'])
                                                    && true === (bool) $typeArray['columnsOverrides'][$fieldName]['config']['enableRichtext']
                                                ) {
     ...
     * @return array Modified TCA
     */
    private function migrateSuggestWizardTypeGroup(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && ('group' === $fieldConfig['config']['type']
                                                                  ...
                                                                  && 'db' === $fieldConfig['config']['internal_type'])
                    ) {
                        ...
                        if (isset($fieldConfig['config']['wizards']) && is_array($fieldConfig['config']['wizards'])) {
                            foreach ($fieldConfig['config']['wizards'] as $wizardName => $wizardConfig) {
                                if (isset($wizardConfig['type']) && 'suggest' === $wizardConfig['type']) {
                            ...
                            foreach ($tableDefinition['types'] as $typeName => &$typeArray) {
                                if (isset($typeArray['columnsOverrides']) && is_array($typeArray['columnsOverrides'])) {
                                    if (isset($typeArray['columnsOverrides'][$fieldName]['config']['wizards'])
                                        ...
                                        foreach ($typeArray['columnsOverrides'][$fieldName]['config']['wizards'] as $wizardName => $wizard) {
                                            if (isset($wizard['type']) && 'suggest' === $wizard['type']
                                            ) {
                                                unset($wizard['type']);
     ...
     * @return array Modified TCA
     */
    private function migrateOptionsOfTypeGroup(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && 'group' === $fieldConfig['config']['type']) {
                        ...
                        if (isset($fieldConfig['config']['show_thumbs'])) {
                            if (false === (bool) $fieldConfig['config']['show_thumbs'] && 'db' === $fieldConfig['config']['internal_type']) {
                            ...
                            } elseif (false === (bool) $fieldConfig['config']['show_thumbs'] && 'file' === $fieldConfig['config']['internal_type']) {
     ...
     * @return array Modified TCA
     */
    private function migrateSelectShowIconTable(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && ('select' === $fieldConfig['config']['type']
                                                                  && isset($fieldConfig['config']['renderType'])
                                                                  && 'selectSingle' === $fieldConfig['config']['renderType'])
                    ) {
                        if (isset($fieldConfig['config']['showIconTable'])) {
                            if (true === (bool) $fieldConfig['config']['showIconTable']) {
     ...
     * @return array
     */
    private function migrateImageManipulationConfig(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && 'imageManipulation' === $fieldConfig['config']['type']) {
     ...
     * @return array
     */
    private function migrateinputDateTimeMax(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['renderType']) && 'inputDateTime' === $fieldConfig['config']['renderType']) {
     ...
     * @return array
     */
    private function migrateInlineOverrideChildTca(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (isset($tableDefinition['types']) && is_array($tableDefinition['types'])) {
                foreach ($tableDefinition['types'] as $typeName => &$typeConfig) {
                    ...
                    foreach ($typeConfig['columnsOverrides'] as $fieldName => &$fieldConfig) {
                        if (isset($fieldConfig['config']['overrideChildTca'])
                            || (isset($fieldConfig['config']['type']) && 'inline' !== $fieldConfig['config']['type'])
                            || (!isset($fieldConfig['config']['type']) && (empty($tca[$table]['columns'][$fieldName]['config']['type']) || 'inline' !== $tca[$table]['columns'][$fieldName]['config']['type']))
                        ) {
            ...
            if (isset($tableDefinition['columns']) && is_array($tableDefinition['columns'])) {
                foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                    if (isset($fieldConfig['config']['type']) && 'inline' !== $fieldConfig['config']['type']) {
                        continue;
                    }
                    if (isset($fieldConfig['config']['foreign_types']) && is_array($fieldConfig['config']['foreign_types'])) {
     ...
     */
    private function migrateLocalizeChildrenAtParentLocalization(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                ...
                if (null === $localizeChildrenAtParentLocalization) {
                    continue;
                }
     ...
     * @return array the modified TCA structure
     */
    private function migratePagesLanguageOverlayRemoval(array $tca)
    {
     ...
     */
    private function deprecateTypeGroupInternalTypeFile(array $tca): array
    {
        foreach ($tca as $table => $tableDefinition) {
            if (!isset($tableDefinition['columns']) || !is_array($tableDefinition['columns'])) {
                continue;
            ...
            foreach ($tableDefinition['columns'] as $fieldName => $fieldConfig) {
                if (isset($fieldConfig['config']['type'], $fieldConfig['config']['internal_type'])
                    && 'group' === $fieldConfig['config']['type']
                    && ('file' === $fieldConfig['config']['internal_type'] || 'file_reference' === $fieldConfig['config']['internal_type'])
                ) {
     ...
     */
    private function sanitizeControlSectionIntegrity(array $tca): array
    {
        $controlSectionNames = [
            'origUid',
            'languageField',
