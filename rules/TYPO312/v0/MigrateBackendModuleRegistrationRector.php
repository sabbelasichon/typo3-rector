...
CODE_SAMPLE
    ...
    public function refactor(Node $node): ?int
    {
        ...
        if ($this->filesystem->fileExists($newConfigurationFile)) {
            $existingFile = new File($newConfigurationFile, $this->filesystem->read($newConfigurationFile));
            ...
            $innerArrayItems[] = new ArrayItem(new String_($main), new String_('parent'));
            $innerArrayItems[] = new ArrayItem(new String_($this->getAccess($moduleArray['access'])), new String_('access'));
            $newArrayItem = new ArrayItem(new Array_($innerArrayItems), new String_($moduleIdentifier));

            $existingFile->changeHasChanged(\false);

            $tempFile = new File('php://temp', <<<CODE
<?php
...
CODE);
            ...
            // Merge the arrays
            $this->mergeFiles_($existingFile, $tempFile); // FIXME: This does NOT work
            $this->mergeFiles($existingFile, $newArrayItem); // This works but creates a one-liner array
...
//        }
    ...
    private function mergeFiles_(File $existingFile, File $newFile): void
    {
        ...
        $existingArray = $this->getNodeArray($fileWithoutNamespace);
        ...
        $newArray = $this->getNodeArray($fileWithoutNamespace2);
    ...
    private function mergeFiles(File $existingFile, ArrayItem $arrayItem): void
    {
        $existingStmts = $existingFile->getNewStmts();
        ...
        // Merge the two arrays
        $existingArray->items[] = $arrayItem;

        $existingFile->changeNewStmts($existingStmts);
    }

    private function getNodeArray(FileWithoutNamespace $fileWithoutNamespace): Array_
    {
        /** @var Return_ $return */
        $return = $fileWithoutNamespace->stmts[0];
