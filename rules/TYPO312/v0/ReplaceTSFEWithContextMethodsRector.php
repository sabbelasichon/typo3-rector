 ...
 */
final class ReplaceTSFEWithContextMethodsRector extends AbstractRector implements DocumentedRuleInterface
{
    /**
     * @var string
     */
    ...
    public function refactor(Node $node): ?Node
    {
        ...
        if ($this->isName($node->name, 'isBackendUserLoggedIn')) {
            $arguments = [
                new String_('backend.user'),
                new String_('isLoggedIn'),
        ...
        if ($this->isName($node->name, self::IS_USER_OR_GROUP_SET)) {
            return $this->nodeFactory->createMethodCall(
                $this->nodeFactory->createMethodCall($contextCall, 'getAspect', [new String_('frontend.user')]),
                self::IS_USER_OR_GROUP_SET
            );
        }

        $userAspectCall = $this->nodeFactory->createMethodCall(
        ...
        return $this->nodeFactory->createMethodCall(
            $contextCall,
            'setAspect',
            [new String_('frontend.user'), $userAspectCall]
        );
    }
